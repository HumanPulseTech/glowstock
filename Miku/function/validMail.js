...
module.exports = async function validMail(token, socket) {
    ...
    try {
        ...
        const users = await connexion.query('SELECT id, pending_offer_id FROM users WHERE verification_token = ? AND email_verified = 0', [token]);
        if (!users.length) return socket.emit('verification error', 'Ce lien est invalide ou a déjà été utilisé.');
        await connexion.query('UPDATE users SET email_verified = 1, verification_token = NULL, verified_at = NOW(), Token = NULL WHERE id = ?', [users[0].id]);
        socket.request.session.userId = users[0].id;
        socket.request.session.role = 'user';
        socket.request.session.save((error) => error ? socket.emit('verification error', 'Impossible de créer la session.') : socket.emit('connection ac', {
