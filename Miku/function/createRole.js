...
module.exports = async function createRole(input, socket) {
  ...
  try {
    ...
    if (!(await canManageRole(connexion, userId, slug, permissions))) return socket.emit('admin error', 'Tu ne peux pas déléguer des droits que tu ne possèdes pas.');
    await connexion.query('INSERT INTO roles (slug, nom, permissions) VALUES (?, ?, ?)', [slug, nom, JSON.stringify(permissions)]);
